document.addEventListener("DOMContentLoaded", () => {
    ...
    let isListening = false; // For voice recognition
    ...
    function initializeSpeechRecognition() {
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            recognition.continuous = false; // Set to false for auto-submit
            ...
            recognition.onstart = () => {
                ...
                // Clear the input field when starting and show listening indicator
                userInput.value = '';
            ...
            recognition.onresult = (event) => {
                ...
                // Auto-submit after a short delay to show the user what was recognized
                setTimeout(() => {
                ...
                }, 500); // 500ms delay to show the transcribed text
            };
            ...
            recognition.onerror = (event) => {
                ...
                userInput.placeholder = 'Speech recognition error. Try again.';
    ...
    // Add visual feedback during speech recognition
    ...
    function handleSpecialEvents(eventType, data) {
        switch(eventType) {
            case 'emotion':
                ...
                // Keep only last 10 emotion records
    ...
    function acceptSuggestion(suggestionType, index) {
        // Handle suggestion acceptance with more context
        ...
        const suggestionElements = document.querySelectorAll('.proactive-suggestions');
    ...
    // ADDED: Quick logout function for programmatic use
    ...
    function setupEventListeners() {
        // CORRECTED: Theme toggle with proper logic
        if (themeToggle && themeIcon) {
            themeToggle.addEventListener('click', () => {
                console.log('Theme toggle clicked');

                // Toggle the light-theme class (dark is default)
                document.body.classList.toggle('light-theme');
                const isLight = document.body.classList.contains('light-theme');

                // Icon shows what theme you'll GET when clicked
                // Dark mode (default): Show ☀️ (click to get light)
                // Light mode: Show 🌙 (click to get dark)
        ...
        // Enhanced microphone button for voice input with auto-submit
        if (micButton) {
            micButton.addEventListener('click', () => {
                ...
                if (isListening) {
                    ...
                    // Only start if not currently generating a response
                    if (!isGenerating) {
                        recognition.start();
                    } else {
        ...
        if (clearChatButton) {
            clearChatButton.addEventListener('click', () => {
                if (confirm('Are you sure you want to clear this conversation?')) {
                    chatBox.innerHTML = '<div class="welcome-message">Hello! How can I help you today? Ask me anything, or try uploading an image.</div>';
                    emotionHistory = [];
                    proactiveSuggestions = [];
                }
            });
        ...
        // ADDED: Setup logout confirmation for all logout buttons
        setupLogoutConfirmation();

        // ADDED: Keyboard shortcut for logout (Ctrl+Shift+L)
        ...
        // Chat form submission with enhanced error handling
        if (chatForm) {
    ...
    // CORRECTED: Initial Page Setup with proper theme logic
    function initializePage() {
        ...
        if (savedTheme === 'light') {
            ...
            if (themeIcon) themeIcon.textContent = '🌙'; // Shows moon in light mode (click to get dark)
        } else {
            // Default dark theme (no class needed)
            document.body.classList.remove('light-theme');
            if (themeIcon) themeIcon.textContent = '☀️'; // Shows sun in dark mode (click to get light)
        }
    ...
    // ADDED: Expose logout function globally for external use
